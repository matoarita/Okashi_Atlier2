		...
		foreach (string asset in importedAssets) {
			...
			using (FileStream stream = File.Open (filePath, FileMode.Open, FileAccess.Read, FileShare.ReadWrite)) {
				...
				foreach(string sheetName in sheetNames) {
					...
					cell = row.GetCell(9); p.cmpitem_kosu1 = (int)(cell == null ? 0 : cell.NumericCellValue);
					cell = row.GetCell(10); p.cmpitem_kosu2 = (int)(cell == null ? 0 : cell.NumericCellValue);
					cell = row.GetCell(11); p.cmpitem_kosu3 = (int)(cell == null ? 0 : cell.NumericCellValue);
					cell = row.GetCell(12); p.cmp_flag = (int)(cell == null ? 0 : cell.NumericCellValue);
					cell = row.GetCell(13); p.cost_time = (int)(cell == null ? 0 : cell.NumericCellValue);
					cell = row.GetCell(14); p.success_rate = (int)(cell == null ? 0 : cell.NumericCellValue);
					cell = row.GetCell(15); p.renkin_Bexp = (int)(cell == null ? 0 : cell.NumericCellValue);
					cell = row.GetCell(16); p.Comment = (cell == null ? "" : cell.StringCellValue);
					cell = row.GetCell(17); p.Comment2 = (cell == null ? "" : cell.StringCellValue);
					cell = row.GetCell(18); p.Comment3 = (cell == null ? "" : cell.StringCellValue);
						s.list.Add (p);
					}
					data.sheets.Add(s);
				}
