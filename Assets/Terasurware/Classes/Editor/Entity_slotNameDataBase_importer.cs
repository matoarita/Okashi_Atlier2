...
public class Entity_slotNameDataBase_importer : AssetPostprocessor {
	private static readonly string filePath = "Assets/Resources/Excel/Entity_slotNameDataBase.xlsx";
	private static readonly string exportPath = "Assets/Resources/Excel/Entity_slotNameDataBase.asset";
		...
		foreach (string asset in importedAssets) {
			...
			using (FileStream stream = File.Open (filePath, FileMode.Open, FileAccess.Read, FileShare.ReadWrite)) {
				...
				foreach(string sheetName in sheetNames) {
					...
					cell = row.GetCell(5); p.slot_getgirllove = (int)(cell == null ? 0 : cell.NumericCellValue);
					cell = row.GetCell(6); p.slot_money = (int)(cell == null ? 0 : cell.NumericCellValue);
					cell = row.GetCell(7); p.slot_beauty = (int)(cell == null ? 0 : cell.NumericCellValue);
						s.list.Add (p);
					}
					data.sheets.Add(s);
				}
