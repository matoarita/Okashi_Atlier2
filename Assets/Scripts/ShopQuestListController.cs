        ...
        for (i = 0; i < quest_database.questTakeset.Count; i++)
        {
            switch (GameMgr.Scene_Name)
            {
                case "Or_Bar_A1":
                    ...
                    if(quest_database.questTakeset[i].Quest_AreaType == 10)
                    {
                        DrawContents();
                    }
                    break;
                ...
                case "Or_Bar_B1":
                    ...
                    if (quest_database.questTakeset[i].Quest_AreaType == 20)
                    {
                        DrawContents();
                    }
                    break;
                ...
                case "Or_Bar_C1":
                    ...
                    if (quest_database.questTakeset[i].Quest_AreaType == 30)
                    {
                        DrawContents();
                    }
                    break;
                ...
                case "Or_Bar_D1":
                    ...
                    if (quest_database.questTakeset[i].Quest_AreaType == 40)
                    {
                        DrawContents();
                    }
                    break;
    ...
    void SelectQuestList(int _id)
    {
        ...
        while (j < quest_database.questset.Count)
        {
            if (quest_database.questset[j].Quest_ID >= _id)
            {
                ...
                //quest_database.questset[j].QuestHyoujiHeartは、人気度として使用。quest_database.questset[j].QuestHyoujiは現在未使用だが、なにかで使ってもいい。
                if (quest_database.questset[j].QuestHyouji <= 99)
                {
                    if (quest_database.questset[j].QuestHyoujiHeart <= PlayerStatus.player_ninki_param) //ランクが足りてるかつ、ハートLVが足りてるものを表示
                    {                        
