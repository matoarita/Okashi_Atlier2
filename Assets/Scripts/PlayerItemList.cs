...
public class PlayerItemList : SingletonMonoBehaviour<PlayerItemList>
{
    ...
    void Start () {
        ...
        while (sheet_no < excel_eventitemdatabase.sheets.Count)
        {
            ...
            while (count < excel_eventitemdatabase.sheets[sheet_no].list.Count)
            {
                ...
                //ここでリストに追加している
                eventitemlist.Add(new ItemEvent(_id, ev_itemName, ev_itemNameHyouji, ev_cost, ev_sell, ev_kosu));
            ...
            if (sheet_no < excel_eventitemdatabase.sheets.Count)
            {
                ...
                {
                    eventitemlist.Add(new ItemEvent(_id + i + 1, "", "", 0, 0, 0)); //エクセルに登録されていないアイテムID分、空をいれている。
                }
            }
        }
    ...
    public void addOriginalItem(string _name, int _mp, int _day, int _quality, int _rich, int _sweat, int _bitter, int _sour, int _crispy, int _fluffy, int _smooth, int _hardness, int _jiggly, int _chewy, int _powdery, int _oily, int _watery, int _girl1_like, int _cost, int _sell, string _tp01, string _tp02, string _tp03, string _tp04, string _tp05, string _tp06, string _tp07, string _tp08, string _tp09, string _tp10, int _itemkosu)
    {
