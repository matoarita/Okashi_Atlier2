    ...
    void Update () {
        ...
        if (move_anim_end == true)
        {
            ...
            switch (GameMgr.Select_place_name)
            {
                ...
                case "Or_Hiroba_Summer_SodaIsland":
                ...
                case "Or_Hiroba1_HotSpring":

                    GameMgr.SceneSelectNum = 172;
                    FadeManager.Instance.LoadScene("Or_Hiroba1", GameMgr.SceneFadeTime);
                    break;
            ...
            case true: //決定が押された
                ...
                if (!GameMgr.OnCatGetMaterial_modeON) //通常の採取画面としての処理
                {
                    ...
                    else
                    {
                        ...
                        time_controller.SetMinuteToHour(GameMgr.Select_place_day, 0, 1 ,false); //材料採取なので、ヒカリのお菓子制作時間は減らない 声がなってしまうので、猫チェックもしない
