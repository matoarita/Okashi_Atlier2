...
public class NPC_Hiroba_Main_Controller : MonoBehaviour {
    ...
    {
        //今いるシーン番号を指定
        GameMgr.Scene_Category_Num = 170;
        ...
        farm_select = canvas.transform.Find("HirobaHouse_Select").gameObject;
    ...
    public void UpdateFarmScene()
    {
        if (!StartRead) //シーン最初だけ読み込む
        {
            ...
            if (GameMgr.SceneMoveAfter_Koushin)
            {
                ...
                time_controller.SetMinuteToHour(GameMgr.SceneMoveAfter_TimeParam, 1, 0, false);
    ...
    {
        matplace_database.matPlaceKaikin("Or_Hiroba_Summer_SweetsHouse"); //牧場解禁

        if (!GameMgr.NPCHiroba_eventList[1240]) //はじめて
        {
            GameMgr.NPCHiroba_eventList[1240] = true;

            GameMgr.scenario_ON = true;
            GameMgr.hiroba_event_placeNum = 1622; //広場でのイベント場所番号
            GameMgr.hiroba_event_ID = 0; //イベントID
    ...
    public void OnCheck_3() //アイテムをあげる
    {
        if (farm_toggle_present.GetComponent<Toggle>().isOn == true)
        {
            ...
            GameMgr.KoyuJudge_num = GameMgr.NPC_OkashiJudge_num[3];//GirlLikeSetの番号を直接指定
